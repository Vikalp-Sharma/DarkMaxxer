...
class Api:
    ...
    def _process_tools(self, response_text, user_prompt=None):
        ...
        create_pattern = re.compile(r'\[TOOL:\s*CREATE_FILE[\s:|]+([^\]\s]+)\]?(.*?)(?=\[/TOOL\]|\[TOOL:|$)', re.DOTALL | re.IGNORECASE)
        edit_pattern = re.compile(r'\[TOOL:\s*EDIT_FILE[\s:|]+([^\]\s]+)\]?(.*?)(?=\[/TOOL\]|\[TOOL:|$)', re.DOTALL | re.IGNORECASE)
        ...
        cmd_pattern = re.compile(r'\[TOOL:\s*RUN_COMMAND\]?(.*?)(?=\[/TOOL\]|\[TOOL:|$)', re.DOTALL | re.IGNORECASE)
    ...
    try:
        ...
        if "QT or GTK" in err_str or "WebViewException" in type(e).__name__:
            fix_msg = (
                ...
                "  sudo rm -rf C:/Program Files/DarkMaxxer/venv && darkmaxxer"
            )
        else:
